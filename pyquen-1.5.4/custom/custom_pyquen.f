...
*==============================================================================
      ...
      open(12,FILE='custom_pyquen.cmnd',STATUS='OLD',IOSTAT=ios,ACTION='READ')

      IF( ios/=0) THEN
       WRITE(6,*) 'Error opening input file custom_pyquen.cmnd.'
       STOP
      ENDIF

      IF( ios/=0) THEN
       WRITE(6,*) 'Error opening output file custom_pyquen.out.'
       STOP
      ENDIF
...
* set initial test values and its rms 
      ...
      onlystable=0d0
      READ (12,*) onlystable
      write(6,*) '#[pyquen] ifb = ',ifb
      write(6,*) '#[pyquen] bfix = ',bfix
      write(6,*) '#[pyquen] quench = ',quench
      write(6,*) '#[pyquen] ienglu = ',ienglu
      write(6,*) '#[pyquen] ianglu = ',ianglu
      write(6,*) '#[pyquen] listpar = ',listpar
      write(6,*) '#[pyquen] onlystable = ',onlystable
      write(6,*) '#[pyquen] RUN for', ntot
      seed=0
      call random_number(seed)  ! for double precision
      write(6,*) '#[pyquen] RUN ', seed
...
* add current test value of pt and its rms 
	...
	ptrms=ptrms+(pt-pta0)**2
      ...
      IF (listpar/=0) THEN
       write(6,*) '#[pyquen] START Event #',ne,'    Impact parameter',bgen,'fm'
c       write(6,*) 'pyhepc called.'
c       call pyhepc(1)
       call pylist(listpar)
       write(6,*) '#[pyquen] END Event #',ne
      ENDIF
...
* and generated one's (with statistical errors) 
      ...
      write(6,1) dna0
1     format(2x,'#[pyquen] True mean multiplicity =',d10.3) 
      write(6,2) dnam, dnrms 
2     format(2x,'#[pyquen] Generated mean multiplicity      =',d10.3,3x,
     > '+-  ',d9.2) 
      write(6,5) pta0
5     format(2x,'#[pyquen] True mean transverse momentum =',d9.2)   
      write(6,6) ptam, ptrms 
6     format(2x,'#[pyquen] Generated mean transverse momentum      =',d9.2,3x,
     > '+-  ',d9.2)    
      call pystat
      end
*******************************************************************************
